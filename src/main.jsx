  ...
  return <Page title="Today" desc="A local command center for commitments, time pressure, reminders, calendar prep, and the next honest use of the day." wide action={<div className="row tight-row"><Button icon="briefs" onClick={() => setRoute("briefs")} disabled={!!viewBriefDisabledReason} title={viewBriefDisabledReason || "View the latest day-plan brief"}>View Brief</Button><Button icon="run" kind="accent" onClick={generateDayPlan} disabled={!!workflowDisabledReason} title={workflowDisabledReason || "Save pending Today input and generate a day plan"}>Generate Day Plan</Button></div>}>
    ...
    <div className="time-layout">
      ...
      <section className="panel">
        ...
        {activeCommitments.length ? activeCommitments.map((item) => <ListRow key={item.id} title={item.title} sub={item.notes || item.status} right={<div className="row tight-row"><Button icon="check" aria-label={`Mark ${item.title || "commitment"} done`} title={`Mark ${item.title || "this commitment"} done`} onClick={() => completeDailyCommitment(item)}>Done</Button><Button icon="x" aria-label={`Remove ${item.title || "commitment"} from Today's Three`} title={`Remove ${item.title || "this commitment"} from Today's Three`} onClick={() => removeDailyCommitment(item)}>Remove</Button></div>} />) : <Empty icon="check" title="No commitments selected" body="Accept up to three high-leverage suggestions or add one from Planner." />}
  ...
  return <Page title="Planner" desc="Capture obligations, map leverage, and keep personal dates beside work context." wide>
    ...
    <div className="split">
      ...
      <section className="panel">
        ...
        {(time.tasks || []).length ? time.tasks.map((item) => <ListRow key={item.id} title={item.title} sub={`${item.leverageCategory || "admin"} · ${item.status || "inbox"} · ${item.estimateMinutes || 30} min`} right={<div className="row tight-row"><Button icon="check" aria-label={`Mark ${item.title || "task"} done`} title={`Mark ${item.title || "this task"} done`} onClick={() => completeTask(item)}>Done</Button><Button icon="x" aria-label={`Archive ${item.title || "task"}`} title={`Archive ${item.title || "this task"}`} onClick={() => archiveTask(item)}>Archive</Button></div>} />) : <Empty icon="planner" title="No tasks captured" body="Add one task to start giving the ranking engine something real to work with." />}
      </section>
    </div>
  ...
  return <Page title="Reminders" desc="Local-first reminders with explicit channel switches and no surprise delivery." wide>
    ...
    <div className="split time-section">
      ...
      <section className="panel">
        ...
        {(time.reminders || []).map((reminder) => <ListRow key={reminder.id} title={reminder.title} sub={reminder.nextOccurrence ? `${reminder.type === "sporadic" ? "sporadic" : reminder.scheduleType} · next ${reminder.nextOccurrence.dateKey} ${reminder.nextOccurrence.localTime}` : reminder.type === "sporadic" ? "sporadic window" : reminder.scheduleType} right={<div className="row tight-row"><Button icon={reminder.enabled ? "x" : "check"} aria-label={`${reminder.enabled ? "Disable" : "Enable"} ${reminder.title || "reminder"}`} title={`${reminder.enabled ? "Disable" : "Enable"} ${reminder.title || "this reminder"}`} onClick={() => toggleReminder(reminder)}>{reminder.enabled ? "Disable" : "Enable"}</Button><Button icon="clock" aria-label={`Pause ${reminder.title || "reminder"} until tomorrow`} title={`Pause ${reminder.title || "this reminder"} until tomorrow`} onClick={() => updateReminder(reminder, { pausedUntil: new Date(Date.now() + 86400000).toISOString() }, `${reminder.title} paused until tomorrow.`)}>Pause</Button><Button icon="x" aria-label={`Archive ${reminder.title || "reminder"}`} title={`Archive ${reminder.title || "this reminder"}`} onClick={() => archiveReminder(reminder)}>Archive</Button></div>} />)}
      </section>
    </div>
  </Page>;
}
  ...
  return <Page title="Reviews" desc="Morning, midday, weekly, monthly, quarterly, and annual review templates." wide>
    <section className="panel">
      ...
      <div className="review-grid">{(time.reviews || []).map((review) => <div className="time-card" key={review.id}>
        ...
        <Button icon={review.enabled ? "x" : "check"} aria-label={`${review.enabled ? "Disable" : "Enable"} ${review.title || "review template"}`} title={`${review.enabled ? "Disable" : "Enable"} ${review.title || "this review template"}`} onClick={() => toggleReview(review)}>{review.enabled ? "Disable" : "Enable"}</Button>
      </div>)}</div>
    </section>
  </Page>;
}
...
function Documents({ state, mutate }) {
  ...
  return <Page title="Documents" desc="Saved work product and doctrine corpus. Records are real SQLite documents and optional chunks." wide>
    ...
    <div className="split">
      ...
      <div className="card table-card"><h2>Corpus</h2>{state.documents.length ? state.documents.map((d) => <ListRow key={d.id} title={d.title} sub={`${d.type} · ${d.wordCount} words · ${d.visibility}`} right={<Button icon={d.status === "active" ? "x" : "check"} onClick={() => updateDocumentStatus(d)} aria-label={`${d.status === "active" ? "Archive" : "Reactivate"} ${d.title || "document"} ${d.status === "active" ? "from" : "for"} active retrieval context`} title={`${d.status === "active" ? "Archive" : "Reactivate"} ${d.title || "this document"} ${d.status === "active" ? "from" : "for"} active retrieval context`}>{d.status === "active" ? "Archive" : "Reactivate"}</Button>} />) : <Empty icon="documents" title="No documents yet" body="Create a document by pasting text into the form before retrieval can affect workflow output." />}</div>
    </div>
  </Page>;
}
...
function Onboarding({ state, mutate, refresh }) {
  ...
  return <div className="onboarding-shell">
    <aside className="onboarding-rail">
      <PillarBriefLockup />
      <div className="onboarding-progress">{steps.map((id, index) => <button key={id} className={index === stepIndex ? "active" : index < stepIndex ? "done" : ""} aria-current={index === stepIndex ? "step" : undefined} onClick={() => go(id)}><b>{index + 1}</b><span>{stepLabels[id] || id}</span></button>)}</div>
    </aside>
...
function Telegram({ state, mutate, refresh }) {
  ...
  const runCmd = async () => {
    if (!window.confirm(`Run Telegram command "${cmd}" now? This may approve or change local workflow state just like a Telegram message.`)) return;
    setTelegramMessage("");
    setResult("");
    try {
  ...
  const telegramSaveDisabledReason = form.enabled && !String(form.botToken || "").trim()
    ? "Paste a Telegram bot token before saving"
    : form.enabled && !String(form.chatId || "").trim()
      ? "Add a Telegram chat ID before saving"
      : "";
  const telegramTestDisabledReason = !state.telegram.enabled || !state.telegram.botToken || !state.telegram.chatId
    ? "Save Telegram bot token and chat ID before sending a test"
    : "";
  const telegramCommandDisabledReason = !cmd
    ? "No Telegram command selected"
    : "";
...
function Settings({ state, mutate, refresh, desktopUpdate }) {
  ...
  const settingsGoogleCalendarTestDisabledReason = !googleCalendarConnected
    ...
    : "";
  const settingsGoogleCalendarSaveDisabledReason = !googleCalendarSelectionDirty
    ? "Change selected calendars before saving"
    : "";
  ...
  >
    ...
    {googleCalendarModal && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeGoogleCalendarModal(); }}>
      <form className="modal-card connector-modal form" role="dialog" aria-modal="true" aria-label="Google Calendar setup" data-modal-focus-trap="true" tabIndex="-1" onSubmit={startGoogleCalendarOAuth}>
        <div className="modal-head"><div><h2>Google Calendar</h2><p>Connect calendar access so Pillar Time can read today's agenda and create approved schedule blocks.</p></div><button type="button" aria-label="Close Google Calendar setup" onClick={closeGoogleCalendarModal} autoFocus><Icon name="x" /></button></div>
        <div className="notice">
